...
function showDialog(options) {
    ...
    if (options.title != null) {
        $('<h5>' + options.title + '</h5>').appendTo(content).addClass("mdl-typography--headline");
    }
    if (options.text != null) {
        $('<p>' + options.text + '</p>').appendTo(content).addClass("mdl-card__supporting-text");
    }
    if (options.neutral || options.negative || options.positive) {
        ...
        if (options.negative) {
            ...
            var negButton = $('<button class="mdl-button mdl-js-button\n' +
                '                        mdl-button--raised mdl-js-ripple-effect mdl-button--accent" id="' + options.negative.id + '">' + options.negative.title + '</button>').css({"float": "left"});
        ...
        if (options.positive) {
            ...
            var posButton = $('<button class="mdl-button mdl-js-button\n' +
                '                        mdl-button--raised mdl-js-ripple-effect mdl-button--accent" id="' + options.positive.id + '">' + options.positive.title + '</button>');
